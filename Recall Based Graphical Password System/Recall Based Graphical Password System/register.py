...
curr = conn.cursor()
# create a table
curr.execute("""CREATE TABLE if not exists record(
...
def col_data():
    if password== "" or impassword=="" or mail=="" or fname=="":
        ...
        try:
            ...
            curr.execute(
                "INSERT INTO record VALUES(:email,:colors,:img,:fname)",
                {
                    'email': txt_mail.get(),
                    'colors':password,
                    'img':impassword,
...
password=""
impassword=""
#concatenate color password
def get_button(t):
    global password
    password+=str(t)
    return
#concatenate pic password
def get_buttons(i):
    global impassword
    impassword+=str(i)
    return
...
# frame1.place(x=510, y=130, width=700, height=500)
...
imagel = Image.open("images/kp.png")
